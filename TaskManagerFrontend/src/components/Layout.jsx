    ...
    const StatCard = ({ title, value, icon }) => (
        <div className="p-2 sm:p-3 rounded-xl bg-white shadow-sm border border-purple-100 hover:shadow-md transition-all duration-300 hover:border-purple-100 group">
            <div className="flex items-center gap-2">
                <div className="p-1.5 rounded-lg bg-gradient-to-br from-blue-400/10 to-purple-400/10 group-hover:from-pink-400/20 group-hover:to-purple-400/20 ">
                    {icon}
                </div>
                <div className="min-w-0">
                    <p className="text-lg sm:text-xl font-bold bg-gradient-to-br from-blue-400 to-purple-400 bg-clip-text text-transparent">
                        {value}
                    </p>
                    ...
                    <div className='xl:col-span-1 space-y-4 sm:space-y-2'>
                        <div className="bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-purple-100">
                            <h3 className='text-base sm:text-lg font-semibold mb-3 sm:mb-4 text-gray-800 flex items-center gap-2'>
                                <IoTrendingUp className='w-4 h-4 sm:w-5 sm:h-5 text-purple-500' />
                                Task Statistics
                            </h3>
                            <div className='grid grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6'>
                                <StatCard
                                    ...
                                    icon={
                                        <FaRegCircle className='w-3.5 h-3.5 sm:w-4 sm:h-4 text-purple-500' />
                                    } />
                                <StatCard
                                    title={"Completed"}
                                    value={stats.completedTasks}
                                    icon={
                                        <FaRegCircle className='w-3.5 h-3.5 sm:w-4 sm:h-4 text-green-500' />
                                    } />
                                <StatCard
                                    title={"Pending"}
                                    value={stats.pendingCount}
                                    icon={
                                        <FaRegCircle className='w-3.5 h-3.5 sm:w-4 sm:h-4 text-fuchsia-500' />
                                    } />
                                <StatCard
                                    title={"Completion Rate"}
                                    value={`${stats.completionPercentage}%`}
                                    icon={
                                        <FiZap className='w-3.5 h-3.5 sm:w-4 sm:h-4 text-purple-500' />
                                    } />
                            </div>
                            ...
                            <div className="space-y-2 sm:space-y-3">
                                <div className="flex items-center justify-between text-gray-700">
                                    <span className='text-xs sm:text-sm font-medium flex items-center gap-1.5'>
                                        <FiCircle className='w-2.5 h-2.5 sm:w-3 sm:h-3 text-purple-500 fill-purple-500' />
                                        Task Progress
                                    </span>
                                ...
                                <div className="relative pt-1">
                                    <div className="flex gap-2.5 items-center">
                                        <div className="flex-1 h-2 sm:h-3 bg-purple-100 rounded-full overflow-hidden">
                                            <div className="h-full bg-gradient-to-br from-blue-400 to-purple-400 transition-all duration-500" style={{ width: `${stats.completionPercentage}%` }} />
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="space-y-2 sm:space-y-3">
                                        {tasks.slice(0, 3).map((task) => (
                                            <div key={task._id || task.id} className="flex items-center justify-between p-2 sm:p-3 hover:bg-purple-50/50 rounded-lg transition-colors duration-200 border border-transparent hover:border-purple-100">
                                                ...
                                                <span className={`px-2 py-1 text-xs rounded-full shrink-0 ml-2 ${task.completed ? "bg-green-100 to-green-700" : "bg-blue-100 to-blue-700"}`}>
                                        ...
                                        {tasks.length === 0 && (
                                            <div className="text-center py-4 sm:py-6 px-2">
                                                <div className="w-12 h-12 sm:w-16 sm:h-16 mx-auto sm:mb-4 rounded-full bg-purple-100 flex items-center justify-center">
                                                    <CiClock1 className='w-6 h-6 sm:w-8 sm:h-8 text-purple-500' />
                                                </div>
